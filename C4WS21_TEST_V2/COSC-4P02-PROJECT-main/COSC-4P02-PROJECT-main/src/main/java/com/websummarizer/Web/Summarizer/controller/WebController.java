...
public class WebController {
    ...
    public String getSummary(
        ...
        String codeShortlink = shortlink.codeShort(url);

        HistoryReqAto historyReqAto = new HistoryReqAto(1L, output, codeShortlink, LocalDateTime.now());

        var historyResAto = historyService.addHistory(historyReqAto); //add content, shotlink, localtime to history for fetching

        model.addAttribute("date", dateFormat.format(date));
        model.addAttribute("user", username);
        model.addAttribute("input", input);
        model.addAttribute("output", output+" : "+codeShortlink);
